...
import BeneficiarioValidator from "./BeneficiarioValidator";
...
const Beneficiario = ({id}) => {
  ...
  const ObtenerBeneficiario = async () => {
    const data = await fetchSinToken(`customer/${Number(id)}/beneficiary`);
    const Beneficiarios = await data.json()
    const beneficiario = Beneficiarios.data[0]
    
    if(beneficiario !== undefined){
      getBeneficiario({
        tipo:beneficiario.tipo_identidad,
        numero:beneficiario.numero_identidad,
  ...
  const send = async () => {
    ...
    getEditandoBe(false);
    try {
      await fetchSinToken(`customer/${id}/beneficiary`,Beneficiario, "POST");
      ObtenerBeneficiario();
      Swal.fire({
        icon: "success",
